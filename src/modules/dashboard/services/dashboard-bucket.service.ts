...
export class DashboardBucketService {
  ...
  private truncate(date: Date, granularity: SeriesGranularity, timezone: string): Date {
    const p = this.time.toZonedParts(date, timezone);
    switch (granularity) {
      case SeriesGranularity.HOUR:
        return this.time.zonedHourStart(p.year, p.month, p.day, p.hour, timezone);
      case SeriesGranularity.DAY:
        return this.time.zonedDayStart(p.year, p.month, p.day, timezone);
      case SeriesGranularity.WEEK: {
        // ISO week starts on Monday.
        const dow = this.time.isoWeekday(p.year, p.month, p.day);
        return this.time.zonedDayStart(p.year, p.month, p.day - (dow - 1), timezone);
      }
      case SeriesGranularity.MONTH:
        return this.time.zonedDayStart(p.year, p.month, 1, timezone);
    }
  }
  ...
  private advance(date: Date, granularity: SeriesGranularity, timezone: string): Date {
    switch (granularity) {
      ...
      case SeriesGranularity.DAY:
        return this.time.addDaysInTz(date, 1, timezone);
      case SeriesGranularity.WEEK:
        return this.time.addDaysInTz(date, 7, timezone);
      case SeriesGranularity.MONTH: {
        const p = this.time.toZonedParts(date, timezone);
        return this.time.zonedDayStart(p.year, p.month + 1, 1, timezone);
      }
    }
  }
}
